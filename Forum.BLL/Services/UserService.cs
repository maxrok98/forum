...
namespace Forum.BLL.Services
{
    public class UserService : IUserService
    {
        ...
        {
            return await _userManager.Users.Where(x => x.Id == id).Include(x => x.Chats).Include(x => x.Posts).ThenInclude(x => x.Coments).Include(x => x.Posts).ThenInclude(x => x.Votes).Include(x => x.Subscriptions).ThenInclude(x => x.Thread).Include(x => x.Votes).ThenInclude(x => x.Post).Include(x => x.Calendar).ThenInclude(x => x.Event).FirstAsync();
        }
