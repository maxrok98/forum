...
namespace Forum.Repositories
{
    public class ThreadRepository : Repository<Thread>, IThreadRepository
    {
        ...
        {
            return await _entity.Include(thread => thread.Image).Include(x => x.Posts).Include(t => t.Subscriptions).ToListAsync();
        }
        ...
        public override async Task<Thread> GetAsync(string id)
        {
            ...
            return await (from t in _entity
                          where t.Id == id
                          select t).Include(t => t.Image).Include(x => x.Posts).Include(t => t.Subscriptions).FirstAsync();
        }
    }
}
